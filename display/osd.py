...
class OSDRenderer:
    ...
    def render(
        ...
        def txt(text, pos, font=_F_MED, color=OSD_WHITE):
            draw.text(pos, text, font=font, fill=_bgr2rgb(color),
        ...
        fx       = cx - total_w // 2
        txt(ifps_txt, (fx, 10),              _F_TOP, ifps_col)
        txt(vfps_txt, (fx + ifps_w + gap, 10), _F_TOP, vfps_col)

        # 1. Top Left: LAT / LON
        txt(f"LAT  {lat: .7f}", (30, 30), _F_MED, OSD_WHITE)
        txt(f"LON  {lon: .7f}", (30, 55), _F_MED, OSD_WHITE)
        ...
        txt(rssi_txt,  (W - _text_w(rssi_txt,  _F_TOP) - 20, 10), _F_TOP, OSD_WHITE)
        txt(timer_txt, (W - _text_w(timer_txt, _F_TOP) - 20, 32), _F_TOP, OSD_WHITE)
        if recording:
            txt("● REC", (W - 90, 54), _F_SM, OSD_RED)
        ...
        txt(f"CELL  {cell_v:.2f} v", (cx_left, by),      _F_MED, OSD_WHITE)
        status_txt = "ARMED" if armed else "DISARMED"
        txt(status_txt,               (cx_left, by + 30), _F_LG,  OSD_WHITE)
        txt(f"MODE  {mode}",           (cx_left, by + 58), _F_MED, OSD_WHITE)

        # 5. Center-right Telemetry
        cx_right = W // 2 + 150
        ry = H // 2 - 50
        txt(f"HOME  {dist_home:.0f} M", (cx_right, ry),       _F_MED, OSD_WHITE)
        txt(f"THR   {throttle}%",        (cx_right, ry + 30),  _F_MED, OSD_WHITE)
        txt(f"SAT   {sats}",             (cx_right, ry + 60),  _F_MED, OSD_WHITE)
        txt(f"ALT   {alt:.1f} M",        (cx_right, ry + 90),  _F_MED, OSD_WHITE)
        ...
        txt(alert_text, (cx - atw // 2, H - 36), _F_XL, col)

        # 7. Speed readout (crosshair itself drawn in phase 1)
        txt(f"{speed:.0f}", (cx + 22, cy - 8),  _F_MED, OSD_WHITE)
        txt("km/h",         (cx + 22, cy + 14), _F_SM,  OSD_GRAY)

        # 8. Attitude Telemetry
        txt(f"HDG  {hdg:03.0f}",           (cx - 45, cy + 70),  _F_MED, OSD_WHITE)
        txt(f"P {pitch:+.0f}  R {roll:+.0f}", (cx - 45, cy + 96), _F_SM,  OSD_WHITE)

        # 9. Bounding box labels (corner brackets drawn in phase 1)
        for det in dets:
            x1 = int(det.x1); y1 = int(det.y1); x2 = int(det.x2)
            cx_b  = (x1 + x2) // 2
            label = "HUMAN"
            lw    = _text_w(label, _F_SM)
            txt(label, (cx_b - lw // 2, y1 - 20), _F_SM, OSD_ORANGE)
